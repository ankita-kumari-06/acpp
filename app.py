...
def register_alumnus():
    if request.method == 'POST':
        ...
        if not email or not name or not password or not department or not passout_year:
...
def view_achievement():
    achievements = Alumni_Achievement.query.order_by(Alumni_Achievement.timestamp.desc()).all()
    return render_template('view_achievement.html', achievements=achievements)
...
def alumni_job_postings():
    if request.method == 'POST':
        ...
        if not company or not role or not ctc or not job_location or not content:
            flash('Please fill in all the details before submitting the job posting.', 'danger')
